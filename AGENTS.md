...
## 2. Versión actual: 7.5.0 (TRIVIAX+ Épica 5: accesibilidad universal)

> v7.5 cierra el plan TRIVIAX+ v8.0: accesibilidad DUA nativa en el cliente.
> `js/services/ttsService.js` (Web Speech API, voz es-*), botón 🔊 flotante y
> navegación completa por teclado inyectados en `UIManager.showQuestionModal`
> (todos los tipos de desafío), `aria-live` en feedback y registro, foco
> gestionado para jugar sin mouse. Detalle: `docs/ACCESIBILIDAD.md`.


> v7.4 agrega la capa analítica predictiva: tres perfiles pedagógicos por
> actividad (motor `php/diagnostico_engine.php`, API `diagnostico_*`, sección
> en `estadisticas.php`) y el cliente LLM `php/llm_client.php`
> (Anthropic/Gemini vía `LLM_PROVIDER`/`LLM_API_KEY` en triviax.env) con
> «Sugerir actividades de refuerzo»: genera un sub-proyecto remedial validado,
> o el prompt para el chatbot del docente si no hay IA configurada.
> También habilita la generación directa en `php/lotto_ai.php`.
> Detalle: `docs/DIAGNOSTICO.md`.
...
> `panel/sesion_nueva.php`) y compañeros fantasma locales
> (`js/engines/botEngine.js` + `action=project_accuracy`).
> Detalle: `docs/MODO_TAREA.md`.
>
> v7.1 inauguró el plan TRIVIAX+ v8.0 (`docs/TRIVIAXPLUS.md`) con el metajuego:
...
| Diagnóstico pedagógico + refuerzo con IA | ✅ v1 perfiles + sugerencia LLM/asistente | `php/diagnostico_engine.php`, `php/diagnostico_api.php`, `php/llm_client.php`, `estadisticas.php`, `docs/DIAGNOSTICO.md` |
| Accesibilidad DUA (TTS + teclado + ARIA) | ✅ v1 en modal de desafíos (todos los tipos) | `js/services/ttsService.js`, `js/ui.js`, `docs/ACCESIBILIDAD.md` |
...
| 2026-07-06 | **TRIVIAX+ Épica 5 (v7.5.0): Accesibilidad universal (DUA).** Cierra el plan TRIVIAX+ v8.0. `js/services/ttsService.js` envuelve la Web Speech API (voz es-* local preferida, preferencia en localStorage, degrada sin soporte). Inyección centralizada en `UIManager.showQuestionModal→_setupModalAccessibility` (cubre todos los tipos de desafío sin tocar los renderers): botón 🔊 con aria-label e indicador pulsante que lee enunciado+opciones, flechas/Home/End para recorrer controles (respetando campos de texto), Enter/Space en cards con tabindex inyectado, foco inicial en el primer control y foco automático en «Continuar» al responder; lectura cortada al resolver/cerrar. `aria-live` assertive en feedback, polite en `#game-logs`, `role=group` en opciones, `:focus-visible` de alto contraste y `prefers-reduced-motion`. Sin cambios de BD/backend. Verificado: suite 13/0 y partida real en navegador (foco, flechas, End, TTS activo, focus en Continuar). |
| 2026-07-06 | **TRIVIAX+ Épica 4 (v7.4.0): Diagnóstico pedagógico predictivo.** Sin migración SQL: capa analítica sobre `intentos`/`stats_desafios`. Motor `php/diagnostico_engine.php` con clasificador determinista por umbrales (Comprensión Crítica <67,5 %; Inconsistencia de Aplicación con ≥3 intentos complejos, general ≥50 % y brecha ≥20 pts o complejo <50 %; resto Dominio Avanzado; tipos complejos: drag_drop/code/sequence/matching/hotspot/fill_blank). API `diagnostico_perfiles` y `diagnostico_sugerir` (POST+CSRF+throttle+audit). Cliente LLM `php/llm_client.php` (Anthropic `/v1/messages` y Gemini; config `LLM_PROVIDER`/`LLM_API_KEY`/`LLM_MODEL` en triviax.env; sin config no hay llamadas externas) conectado también a `php/lotto_ai.php`. Sección en `estadisticas.php` con tarjetas de perfil y botón de refuerzo (JSON validado con `triviax_validate_project` o prompt para chatbot). Verificado: `tests/diagnostico_test.php` (22 OK, suite 13/0), E2E HTTP sembrado (11 OK) y sección visual en navegador. |
